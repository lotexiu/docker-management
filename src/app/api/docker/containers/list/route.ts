...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!validation.success) {
      ...
      return createValidationErrorResponse(validation.error.issues[0]?.message || 'Parâmetros inválidos');
    }
    ...
    if (data.filters) {
      try {
        ...
        return createValidationErrorResponse('Formato de filtros inválido (deve ser JSON válido)');
      }
    }

    // Obter todos os containers que correspondem aos filtros
    const allContainers: Dockerode.ContainerInfo[] = await (docker.listContainers(listOptions) as any);
