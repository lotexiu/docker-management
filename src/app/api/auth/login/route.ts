...
export async function POST(request: NextRequest) {
	try {
		...
		const user = usersDatabase.find(
			u => u.username === validatedData.username || u.email === validatedData.username
		);

		// Verifica se o usuário existe e se a senha está correta
		if (!user || user.password !== validatedData.password) {
			// Log de tentativa de login falha (útil para segurança)
			console.log(`Tentativa de login falha - IP: ${clientInfo.ip}, User: ${validatedData.username}`);
		...
		// Login bem-sucedido
		console.log(`Login bem-sucedido - User: ${user.username}, IP: ${clientInfo.ip}`);
